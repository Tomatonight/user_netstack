...
static char* test_ip="192.168.72.140";
void udp_test1()
{
    init();
    ...
    sockaddr_in addr;
    addr.sin_addr.s_addr = inet_addr(test_ip); // 192.168.72.129
...
void udp_test2()
{
    ...
    printf("socket success\n");
    sockaddr_in addr;
    addr.sin_addr.s_addr = inet_addr(eth_ip); 
    addr.sin_port = htons(1234);
    if (bind_(fd, &addr) < 0)
    {
        printf("bind err\n");
...
void tcp_test1()
{
    ...
    sockaddr_in daddr;
    daddr.sin_addr.s_addr = inet_addr(test_ip);
