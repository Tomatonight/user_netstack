...
void udp_manage::remove_sock(sock_udp *sock)
{
    ...
    udp_map.erase({sock->saddr,sock->sport});
}
sock_udp *udp_manage::search_udp_sock(uint32_t s_ip, uint32_t d_ip, uint16_t s_port, uint16_t d_port)
{
    std::unique_lock<std::mutex> l(mtx);
    if (udp_map.find({d_ip,d_port}) == udp_map.end())
    {
        return nullptr;
    }
    sock_udp *sock = udp_map[{d_ip,d_port}];
    if (sock->connect_state == UDP_CONNECT && (sock->dport != s_port  || sock->daddr != s_ip))
    {
        return nullptr;
    }
    return sock;
...
{
    std::unique_lock<std::mutex> l(mtx);
    std::tuple<uint32_t,uint16_t> key({sock->saddr,sock->sport});
    if (udp_map.find(key) != udp_map.end())
    {
        printf("add udp sock error\n");
        return;
    }
    udp_manage::get()->bitmap_udp_port.set_value(sock->sport,true);
    udp_map[key] = sock;
}
...
void udp_recv(skbuff *skb)
{
 //   printf("udp recv\n");
    ...
    udp_hdr *udp = (udp_hdr *)(skb->data_start + ETH_HDR_LEN + ip->ihl*4);
...
int sock_udp::bind_(sockaddr_in *addr)
{
    ...
    if(addr->sin_port!=0&&udp_manage::get()->bitmap_udp_port.test(addr->sin_port))
    {
        printf("udp port err\n");
        return -1;
    }
