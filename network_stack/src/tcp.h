...
public:
    ...
    uint32_t un_ack_seq = 0;       // 未被确认的seq
    ...
    void handle_recv_(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    ...
    void destroy_sock();
    //*******
    bool filter_seq(tcp_hdr *tcp);
    bool updata_ack(tcp_hdr *tcp);
    bool filter_flag(tcp_hdr *tcp);
    bool filter_rst(tcp_hdr *tcp, skbuff *skb);
    void recv_while_establish(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    void recv_while_finwait1(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    void recv_while_finwait2(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    void recv_while_closewait(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    void recv_while_last_ack(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    void recv_while_closing(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
    void recv_while_timewait(skbuff *skb, tcp_hdr *tcp, ip_hdr *ip);
};
